...
void dsgemm(float *c_matrix, const float *a_matrix,
        ...
        if (nnz > 0) {
            ...
            for (int row_ind = 0; row_ind < nnz; row_ind++) {
                for (int i = 0; i < m_dim; i++) {
                    int row = b_matrix_indices[data_ptr + row_ind];
                    c_matrix[i*n_dim+col] += a_matrix[i*k_dim+row] * b_matrix_data[data_ptr+row_ind];
                }
                row_fwd = row_ind;
            }
            data_ptr += row_fwd+1;
